 ...
 * @since  1.0.0
 */
class MenjadorformModel extends FormModel
{
	private $item = null;
	...
	public function getItem($id = null)
	{
		if ($this->item === null)
		{
			...
			if ($table !== false && $table->load($id) && !empty($table->id))
			{
				...
				if ($id)
				{
					$canEdit = $user->authorise('core.edit', 'com_afarosadelsvents.menjador.' . $id) || $user->authorise('core.create', 'com_afarosadelsvents.menjador.' . $id);
				}
				else
				{
				...
				if (!$canEdit && $user->authorise('core.edit.own', 'com_afarosadelsvents.menjador.' . $id))
				{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_afarosadelsvents.menjador', 'menjadorform', array(
						'control'   => 'jform',
						'load_data' => $loadData
				)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_afarosadelsvents.edit.menjador.data', array());
		...
		// Support for multiple or not foreign key field: opcio_menjador
		$array = array();

		foreach ((array) $data->opcio_menjador as $value)
		{
			if (!is_array($value))
			{
				$array[] = $value;
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('menjador.id');
		...
		if ($id)
		{
			...
			$authorised = $user->authorise('core.edit', 'com_afarosadelsvents.menjador.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_afarosadelsvents.menjador.' . $id);
		}
		else
		{
	...
	public function delete($id)
	{
		...
		if (empty($id))
		{
			$id = (int) $this->getState('menjador.id');
		}
		...
		if ($user->authorise('core.delete', 'com_afarosadelsvents.menjador.' . $id) !== true)
		{
				throw new \Exception(Text::_('JERROR_ALERTNOAUTHOR'), 403);
		}
