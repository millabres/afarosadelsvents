...
<div class="menjador-edit front-end-edit">
	...
	<?php echo HTMLHelper::_('uitab.startTabSet', 'myTab', array('active' => 'menjador')); ?>
	<?php echo HTMLHelper::_('uitab.addTab', 'myTab', 'menjador', Text::_('COM_AFAROSADELSVENTS_TAB_MENJADOR', true)); ?>
	<?php echo $this->form->renderField('nom_menjador'); ?>

	<?php
	// Obtiene el formato de fecha del archivo de idioma de Joomla
	$dateFormat = Text::_('JDATE_FORMAT_LC1');
	// Reemplaza 'jform[data_menjador]' con el nombre real del campo de tu fecha
	$dateFieldId = 'jform[data_menjador]';
	?>

	<div class="form-group">
	    <label for="<?php echo $dateFieldId; ?>"><?php echo $this->form->getLabel('data_menjador'); ?></label>
	    <input type="text" class="form-control" id="<?php echo $dateFieldId; ?>" name="<?php echo $dateFieldId; ?>" value="<?php echo JFactory::getDate($this->item->data_menjador)->format($dateFormat); ?>" />

	    <script>
	        // Inicializa Flatpickr para el campo de fecha
	        flatpickr('#<?php echo $dateFieldId; ?>', {
	            dateFormat: '<?php echo str_replace('Y', 'Y', $dateFormat); ?>', // Ajusta el formato según sea necesario
	            enableTime: false, // Establece en true si necesitas incluir la selección de hora
	            // Agrega cualquier otra opción de Flatpickr aquí según sea necesario
	        });
	    </script>
	</div>
